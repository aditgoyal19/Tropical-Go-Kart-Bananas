...
void VehicleSetup::setupVehicleData(const hkpWorld* world, hkpVehicleData& data)
{
	...
	for (int i = 0; i < data.m_numWheels; i++)
	{
		...
		data.m_wheelParams[i].m_radius = 0.1f;
		data.m_wheelParams[i].m_width = 0.1f;
...
void VehicleSetup::setupComponent(const hkpVehicleData& data, hkpVehicleDefaultSteering& steering)
{
	...
	//degrees  
	steering.m_maxSteeringAngle = 75 * (HK_REAL_PI / 180);// previously 35 and steering was very to and fro 
...
{
	engine.m_maxTorque = 2000.0f;

	engine.m_minRPM = 1000.0f;
	engine.m_optRPM = 2000.0f;

	// This value is also used to calculate the m_primaryTransmissionRatio.
	engine.m_maxRPM = 3000.0f;
...
void VehicleSetup::setupComponent(const hkpVehicleData& data, hkpVehicleDefaultTransmission& transmission)
{
	...
	const hkReal vehicleTopSpeed = 120.0f;
	const hkReal wheelRadius = 0.03f;
...
void VehicleSetup::setupComponent(const hkpVehicleData& data, hkpVehicleDefaultSuspension& suspension)
{
	...
	suspension.m_wheelParams[0].m_length = 0.15f;
	suspension.m_wheelParams[1].m_length = 0.15f;
	suspension.m_wheelParams[2].m_length = 0.15f;
	suspension.m_wheelParams[3].m_length = 0.15f;

	const float str = 100.0f;
	...
	//
	{
		const hkReal hardPointFrontX = 0.2f;
		const hkReal hardPointBackX = -0.2f;
		const hkReal hardPointY = -0.005f;
		const hkReal hardPointZLeft = -0.12f;
		const hkReal hardPointZRight = 0.12f;

		suspension.m_wheelParams[0].m_hardpointChassisSpace.set(hardPointFrontX, hardPointY, hardPointZLeft);
		suspension.m_wheelParams[1].m_hardpointChassisSpace.set(hardPointFrontX, hardPointY, hardPointZRight);
		suspension.m_wheelParams[2].m_hardpointChassisSpace.set(hardPointBackX, hardPointY, hardPointZLeft);
		suspension.m_wheelParams[3].m_hardpointChassisSpace.set(hardPointBackX, hardPointY, hardPointZRight);
	}
...
void VehicleSetup::setupComponent(const hkpVehicleData& data, hkpVehicleDefaultAerodynamics& aerodynamics)
{
	...
	aerodynamics.m_extraGravityws.set(0.0f, -6.0f, 0.0f);
}
